...
public class Solution {
    ...
    public Set<Integer> getAllFriendsAndPotentialFriends(int index, int deep) {
        ...
        for (int i = 0; i < humansRelationships[index].length - 1; i++) {
        ...
        for (int i = 0; i < humansRelationships.length; i++) {
            if (humansRelationships[i].length > index + 1) {
        ...
        if (deep > 0) {
            Set<Integer> proxy = new HashSet<>();
            for (Integer i : result) proxy.addAll(getAllFriendsAndPotentialFriends(i, deep));
            result.addAll(proxy);
        }
